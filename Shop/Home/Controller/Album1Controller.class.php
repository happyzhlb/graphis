...
class Album1Controller extends FrontendController{
	var $_mod=null; 
	var $_albumcate_mod=null; 
	var $_albumcate_album_mod=null; 
    function __construct(){
        parent::__construct();
        $this->Album1Controller();
    }
    
    function Album1Controller(){
        $this->_mod=D('Album');  
    ...
    function index(){  
       ...
       $this->display('index/album.index'); 
    }    
    /** 详情页 */
    function detail(){       	
   ...
   function reward(){
   	  ...
   	  if(!IS_POST){
   	  	 ...
   	  	 if(isWeixin() && empty($openid)){ 
   	  	     $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx2b8ec378bf9b09ac&redirect_uri=http%3A%2F%2Fwww.graphis.club&response_type=code&scope=snsapi_base&state=/album1/reward/id/'.$album_id.'.html?code='.I('code').'&connect_redirect=1#wechat_redirect';
   	  	    #echo $url; exit;
   	  	     redirect($url); 
   	  	 }
		  ...
		  $this->assign('album_id',$album_id);
	   	  ...
	   	  #$recommand_album = M('album')->where(array('id<>'.$album_id))->limit(2)->select();
	   	  #$a = array_rand($recommand_album,4); 
	   	  #$recommand_albums[0] = $recommand_album[$a[0]]; 
	   	  #$recommand_albums[1] = $recommand_album[$a[1]]; 
	   	  #$recommand_albums[2] = $recommand_album[$a[2]];
	   	  #$recommand_albums[3] = $recommand_album[$a[3]];
	   	  ...
	   	  $this->display('index/album1.reward');
   	  }else{  //打赏图片列表
