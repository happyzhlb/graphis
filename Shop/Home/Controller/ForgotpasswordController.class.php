...
class ForgotpasswordController extends FrontendController{
    ...
    function index(){
        if(!IS_POST){
            $this->display('./forgotpassword.form');   
        }else{
            $email = trim(I('email'));
            ...
            $url = 'http://'. C('site_url').U('/Forgotpassword/reset',array('fid'=>$fid));
			//发送邮件处理
    ...
    function reset(){
        ...
        if(!IS_POST){
            ...
            $this->display('./forgotpassword.reset');
        }else{
            ...
            $this->success('Password has been successfully reset, please login.',U('/Index/login_register'));
