...
class Album2Controller extends FrontendController{
	var $_mod=null; 
	var $_albumcate_mod=null; 
	var $_albumcate_album_mod=null; 
    function __construct(){
        parent::__construct();
        $this->Album2Controller();
    }
    
    function Album2Controller(){
        $this->_mod=D('Album');  
   ...
   function reward(){
   	  ...
   	  if(!IS_POST){
   	  	 ...
   	  	 if(isWeixin() && empty($openid)){ 
   	  	     $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx2b8ec378bf9b09ac&redirect_uri=http%3A%2F%2Fwww.graphis.club&response_type=code&scope=snsapi_base&state=/album2/reward/id/'.$album_id.'.html?code='.I('code').'&connect_redirect=1#wechat_redirect';
   	  	     redirect($url); 
   	  	 }
		  ...
		  $this->assign('album_id',$album_id);
	   	  ...
	   	  $this->assign('command_code',$ro['command_code']);
	   	  $this->display('index/album2.reward');
   	  }else{  //打赏图片列表  
