...
//     	$this->assign('next_list',$next_list);
   ...
   function reward(){
   	  ...
   	  if(!IS_POST){ 
   	     $isVip = false;
   	     if(strtolower($_SERVER['HTTP_HOST']) == 'vip.graphis.club') {
   	         if(!session('user_id')){
   	             $this->error('VIP会员专区，请先登录会员.','http://vip.graphis.club/');
   	         }else{ 
   	            if(session('is_vip')=='1' && (strtotime(session('vip_end_date'))>=gmtime() ) ){
   	                $isVip=true;
   	            } 
   	         } 
   	     } 
		  ...
		  $this->assign('album_id',$album_id);
	   	  $find  = $album_mod->where(array('id'=>$album_id,'if_show'=>1))->field('title,reward_fee,pay_index,models_id')->find();
	   	  if(!$find){
	   	      //redirect('/');
	   	      $this->error('该专辑不存在或者已经被删除.','/',3);
	   	  } 
	   	  $this->assign('title',$find['title']);	   	  
	   	  $this->assign('pay_index',$find['pay_index']?$find['pay_index']:3); 
	   	  //模特介绍
	   	  $content = M('models')->where(array('id'=>(int)$find['models_id']))->getField('content');
	   	  $this->assign('content',strip_tags(htmlspecialchars_decode($content)));
	   	  ...
	   	  //调试或者设置打赏金额为0，免打赏
	   	  if(I('debug')=='true' || $find['reward_fee'] == 0 || $isVip) { $isPay = 1;  }
	   	  ...
	   	  if(I('debug')=='true') echo M()->getLastSql();
	   	  foreach ($list as $key => $val){
