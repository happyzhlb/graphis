...
class MemberController extends FrontendController{
    ...
    function _init(){
        ...
        $this->assign('user',$this->visitor->get_detail());
    }
    ...
    function index(){
        ...
        $user_id=$this->visitor->get('user_id');
        $user= $this->_user_mod->where('user_id='.$this->user_id)->find(); 
        $user['grade_name'] = M('user_grade')->where(array('grade_id'=>$user['grade_id']))->getField('grade_name');
