...
class UserAddressController extends MemberController{
    ...
    function index(){ 
       ...
       //shipping address
       $where['type']='shipping'; 
       $shipping=$this->_mod->where($where)->order('address_id desc')->select();  
       $this->assign('shipping',$shipping);
		
       //billing address
       $where['type']='billing'; 
       $billing=$this->_mod->where($where)->order('address_id desc')->select();  
   			...
   			if(!$this->_mod->create($data)){
                ...
                $res=$this->_mod->where($where)->save(); 
              	if(FALSE!==$res){
                	...
                	$this->success('OK',I('referer')?I('referer'):U('index'));
              	}else{
   					$this->error('Failed');
   			  	}  
   			}
    ...
    function delete(){ 
    	...
    	if(I('spec_ids')){
    		$this->redirect(U('UserAddress/index',$this->get_param(),''));
    	}
