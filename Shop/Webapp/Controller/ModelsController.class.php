...
class ModelsController extends FrontendController{
    ...
    function index(){ 
       ...
       $models_list=$this->_mod->field('id,title,ctime,photo,labels,cate_id')->where($where)->limit($page->firstRow.','.$page->listRows)->order('id desc')->select(); 
       foreach ($models_list as $key => $val){
       	$models_list[$key]['album_num'] = M('album')->where(array('models_id'=>$val['id']))->count();
       	 #echo M()->getLastsql();
       }
        
       $this->assign('modelslist',$models_list);  
