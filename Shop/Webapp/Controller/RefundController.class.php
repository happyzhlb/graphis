 ...
 */
 ...
 class RefundController extends MemberController{
    ...
    function view(){ 
        $rec_id = I('id','','intval');
        //$this->redirect(U('edit',array('id'=>$rec_id),''));  //跳转到编辑退款页面
    ...
    /** 申请退款 */
    function apply(){
        $rec_id = I('id','','intval');  #dump($rec_id); exit();
        $order_detail = $this->_get_order_detail($rec_id);
        //判断订单状态是否允许退款
        if(in_array($order_detail['order_status'],array('0','11')) || $order_detail['refund_status']){
            $this->error('Refund not allowed under this order status.');
        ...
        if(!IS_POST){
            ...
            D('')->commit();
            //$this->success('Refund applied successfully.',U('Myorders/view',array('id'=>$rec_id)));
            $this->success('Refund applied successfully.',U('Myorders/index'));
        }
    }
    ...
    function edit(){
        ...
        if(!IS_POST){
            ...
            $this->apply_success();
            //$this->success('Refund information edited successfully.',U('Refund/edit',array('id'=>$rec_id)));
    ...
    function note(){ 
    	$this->display('./Refund.note');
    }
	
    function apply_success(){ 
    	$this->display('./Refund.success');
    }  

	function logistics_information(){
		$this->display('./logistics_information');
	}
