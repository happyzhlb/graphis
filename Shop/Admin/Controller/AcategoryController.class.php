...
class AcategoryController extends BackendController{
    var $_acategory_mod = null;
    function __construct(){
        parent::__construct();
        $this->AcategoryController();
    }
    function AcategoryController(){
        $this->_acategory_mod = D('Acategory');
    }
    
    /** 文章分类 */
    function index(){ 
    	//C('SHOW_PAGE_TRACE',True); 
        //$list = $this->_acategory_mod->get_category(0,true);
		//缓存文章类别
        $acategory=S('acategory');
		if(!$acategory){
			$acategory = $this->_acategory_mod->get_category(0,true);
			S('acategory',$acategory);
		}
        $this->assign('acategory',$acategory); 
        $this->display('./acategory.index');
    }
    ...
    function sel_ad(){ 
        ...
        if(isset($_REQUEST['single'])){
        	$this->display('./acategory.sel.ad');
        }else{
        	$this->display('./acategory.sel');
        }
    ...
    function sel_cate(){ 
        ...
        if(!isset($_REQUEST['multi'])){
        	$this->display('./acategory.selcate');
        }  
    }
    ...
    function ajaxGetCategory(){     
    	$cate_id=I('cate_id');
    	$list=$this->_acategory_mod->get_category($cate_id,true);
    	if($list[0]['children']){
    ...
    function add(){
        if(!IS_POST){
           ...
           if($_FILES['b_photo']['size']>0){  
            ...
            if(!$this->_acategory_mod->create($data)){
                $this->error($this->_acategory_mod->getError());
                return;
            }else{
                if(!$this->_acategory_mod->add($data)){
                    $this->error('分类添加失败');
                    return;
                }
            }
            //清空缓存
            S('acategory',null);
            $this->success('分类添加成功',U('/Admin/Acategory'));   
        }
    }
    ...
    function edit(){
        ...
        $cate = $this->_acategory_mod->get_category($cate_id);
        ...
        if(!IS_POST){
        	...
        	$brand_ids = $this->getBrandStr($cate_id);
        	$gestation = M('gestation')->field('gestation_id,gestation_name,gestation_days')->select();
        	$this->assign('gestation',$gestation); 
        	$this->assign('brand_ids',$brand_ids);
            $this->assign('acategory',$acategory);
            $this->assign('cateinfo',$cate);
            $this->display('./acategory.edit');
        }else{
           ...
           //关联品牌
           $brand_ids = I('brand_ids'); 
           $acate_brand_mod = M('acate_brand');
           $acate_brand_mod->startTrans();
           $brand_arr = explode(',', $brand_ids);
           $acate_brand_mod->where(array('cate_id'=>$cate_id))->delete();
           foreach ($brand_arr as $key => $val){
           		$res = $acate_brand_mod->add(array('cate_id'=>$cate_id,'brand_id'=>$val));
           		if(!$res){
...
//           //关联品牌json
//           $brandJson = $this->getBrandJson($cate_id);            
//           $data['brand_json'] = $brandJson;
           ...
           if($_FILES['b_photo']['size']>0){  
            ...
            if(!$this->_acategory_mod->create($data)){
                ...
                if(FALSE===$this->_acategory_mod->save($data)){
                	$acate_brand_mod->rollback();
                    $this->error('分类编辑失败'.M()->getLastsql());
                    return;
                }
            }
            $acate_brand_mod->commit();
            
            //清空缓存
            S('acategory',null);
            $this->success('分类编辑成功',U('/Admin/Acategory'));
        }   
    }
    ...
    function drop(){
        ...
        $where['is_hold']=0;
        $cate = $this->_acategory_mod->where($where)->select();
        if(empty($cate)){
            $this->error('分类不存在或者Hold状态,删除失败');
            return;
        }
        $this->_acategory_mod->startTrans();
        foreach($cate as $key => $value){
            //删除所有子分类
            if(!$this->_drop_gcategory($value['cate_id'])){
                $this->_acategory_mod->rollback();
                $this->error('子分类删除失败');
                return;
            }
        }
        if(!$this->_acategory_mod->where($where)->delete()){
            $this->_acategory_mod->rollback();
            $this->error('分类删除失败');
            return;
        }
        $this->_acategory_mod->commit();
    ...
    private function _drop_gcategory($id){
        ...
        $where['parent_id'] = $id;
        $cate_list = $this->_acategory_mod->where($where)->select();
        if(empty($cate_list)) //没有子分类，直接返回TREU
            return true;
        $acategory_article_mod=M('category_article');
        foreach($cate_list as $key => $list){
            $this->_acategory_mod->delete($list['cate_id']);
            //$acategory_article_mod->delete($list['cate_id']);  //暂时没用文章类别一对多关系
