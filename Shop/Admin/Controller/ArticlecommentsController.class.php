...
class ArticlecommentsController extends BackendController{
    var $_adplace_mod = null;
    var $_app_mod = null;
    function __construct(){
        parent::__construct();
        $this->ArticlecommentsController();
    }
    function ArticlecommentsController(){ 
        $this->_app_mod = D('ArticleComments');
    }
    ...
    function index(){
        ...
        $this->display('./articleComments.index');
    }
    
    /** 添加举报 */
    function add(){
        if(!IS_POST){  
            $this->display('./articleComments.form');
        }else{
    ...
    function edit(){
        ...
        if(!IS_POST){ 
            $this->assign('list',$list); 
            $this->display('./articleComments.edit');
        }else{
            $version=trim(I('version'));
            if(empty($version)){
                $this->error('举报号不正确，只能为数字格式.');
