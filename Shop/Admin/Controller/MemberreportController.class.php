...
class MemberreportController extends BackendController{
    var $_order_mod = null;
    ...
    function index(){
      if(!IS_POST){
    	$this->display('./report.member.index');
      } 
    } 
    ...
    function goods(){ 
    	$this->_before_index();
    	$pre=C('DB_PREFIX');
    	$sqlcmd="SELECT b.goods_id,b.goods_name,b.goods_amount,b.weight,b.quantity  
    	 FROM {$pre}order a INNER JOIN {$pre}order_goods b ON a.order_id=b.order_id where 1"; 
		$sqlcmd.=$this->where();
        $list=M()->query("SELECT goods_id,goods_name,SUM(weight) weight,sum(goods_amount) goods_amount,sum(quantity) quantity from($sqlcmd) as aa GROUP BY goods_id");
    ...
    function user(){ 
    	$this->_before_index();
    	$pre=C('DB_PREFIX');
    	$sqlcmd="SELECT a.user_id,b.user_name,count(*) cnt,sum(goods_amount) goods_amount,sum(shipping_fee) shipping_fee,sum(integral_fee) integral_fee,sum(discount_fee) discount_fee,sum(totle_fee) totle_fee from {$pre}order a ,{$pre}user b  where a.user_id=b.user_id"; 
