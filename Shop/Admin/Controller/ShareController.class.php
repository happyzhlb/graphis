...
class ShareController extends BackendController{
    ...
    function add(){
        if(!IS_POST){  
            $this->display('./share.form');
        }else{
            $version=trim(I('version'));
            if(empty($version)){
                $this->error('App分享统计号不正确，只能为数字格式.');
            }
            $data = array( 
            ...
            $this->_app_mod->add();
            $this->success('App分享统计添加成功',U('index'));
        }
    }
    ...
    function edit(){
        ...
        if(!$list){
            $this->error('App分享统计不存在或已经被删除');
            return;
        }
        if(!IS_POST){ 
            $this->assign('list',$list); 
            $this->display('./share.edit');
        }else{
            $version=trim(I('version'));
            if(empty($version)){
                $this->error('App分享统计号不正确，只能为数字格式.');
            ...
            $this->success('App分享统计编辑成功',U('index'));
        }
    }
    ...
    function drop(){
        ...
        $this->success('App分享统计删除成功',U('index'));
    }
    ...
    function editstatus(){
        ...
        if(!$this->_app_mod->save($ad)){
            $this->error('App分享统计状态编辑失败');
            return;
        }
        $this->success('App分享统计状态编辑成功.',U('index'));
    }
