...
use Think\Model;
class WikicateModel extends Model{
    ...
    /** 类别-数组结构 */
    function getRelateCate($cate_id,&$n){
        static $return; 
        $acate = $this->where(array('cate_id'=>$cate_id))->Field('cate_id,cate_name,parent_id')->find();
        if($acate){ 
            $return[] = $acate;
            if($acate['parent_id']){
                if(++$n>10) return $return; 
                unset($return); 
                $return[] =  $this->getRelateCate($acate['parent_id'],$n);
            }
        } 
        return $return;
    }
    ...
    /** 类别-树状结构 */
    function getCateTree($cate_id,$return=null,&$n){
        $acate = $this->where(array('cate_id'=>$cate_id))->Field('cate_id,cate_name,parent_id')->find();
        if($acate){
            $return = $acate;
            if($acate['parent_id']){
                if(++$n>5) return $return;
                $return['parent'] =  $this->getRelateCate($acate['parent_id'],$return,$n);
            }
        }
        return $return;
    }    
    ...
    /** 分类-导航栏*/
    function getCateNav($cate_id,$keywords='wiki'){ 
        $nav = $this->getRelateCate($cate_id,$n); 
        for($i=1;$i<=$n;$i++){
            $nav = current($nav);
        }
        foreach ($nav as $v){
            $arr[] = "<a href='".U($keywords.'/index',array('cate_id'=>$v['cate_id']))."' >".$v['cate_name'].'</a>';
        }
        return htmlspecialchars_decode(join(' > ',array_reverse($arr))); 
    }
